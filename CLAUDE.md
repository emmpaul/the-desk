...
- **Hotfixes are the one exception, and only when both halves hold:** a released version is broken in production *and* `develop` cannot be promoted as it stands. Then branch off **`master`**, PR into `master` (title `fix:`, which cuts a patch), and merge the stable release PR. Everything else — including nearly all `fix:` work — goes through `develop`; do not take this route because it is faster. After the release, `master` **must** be merged back into `develop` with a merge commit, or the next promotion carries an older ancestor of those files and silently reverts the fix. The `backmerge` job opens that PR automatically after every stable release. Full path in `CONTRIBUTING.md` → *Hotfixes*.
