...
```bash
./docker/upgrade.sh --target=1.13.0 /srv/backups # x-release-please-version
```
...
```bash
# set APP_VERSION=1.13.0 in .env # x-release-please-version
docker compose pull
docker compose up -d
...
```bash
git fetch --tags
git checkout v1.13.0 # x-release-please-version         (the desired release tag)
docker compose down
