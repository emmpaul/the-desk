...
```bash
./docker/upgrade.sh --target=1.11.1 /srv/backups # x-release-please-version
```
...
```bash
# set APP_VERSION=1.11.1 in .env # x-release-please-version
docker compose pull
docker compose up -d
...
```bash
git fetch --tags
git checkout v1.11.1 # x-release-please-version         (the desired release tag)
docker compose down
