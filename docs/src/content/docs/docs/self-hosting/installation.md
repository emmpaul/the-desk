...
cd the-desk
git fetch --tags && git checkout v1.3.0 # x-release-please-version   (the desired release tag)
...
echo 'APP_IMAGE=ghcr.io/emmpaul/the-desk:1.3.0' >> .env # x-release-please-version
...
cd the-desk
git fetch --tags
git checkout v1.3.0 # x-release-please-version         (the desired release tag)
